...
public class TestVisualization {
	...
	public static void main(String[] args) {
    	...
    	DotVisualizer sqv = new DotVisualizer(
    		//"PREFIX foaf: <http://xmlns.com/foaf/0.1/> SELECT ?b WHERE { { ?a foaf:type ?b } UNION { ?a foaf:type ?c } UNION { ?a foaf:type ?d } }"
    		//"PREFIX foaf: <http://xmlns.com/foaf/0.1/> PREFIX names: <http://names.directory.com/> SELECT ?age WHERE { VALUES (?thing) { ( \"reading\" ) ( \"playing\" ) } ?person foaf:name names:John . ?person foaf:age ?age . ?person foaf:made+ ?thing. FILTER(?age > 18). }  ORDER BY DESC(?thing) LIMIT 10"
