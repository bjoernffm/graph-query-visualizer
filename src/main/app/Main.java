...
public class Main {
    public static void main(String[] args) {
    	...
    	DotVisualizer sqv = new DotVisualizer(
    		...
    		//"PREFIX foaf: <http://xmlns.com/foaf/0.1/> SELECT ?name ?mboxName WHERE  { ?x foaf:name  ?name . OPTIONAL { ?x  foaf:mbox  ?mbox . ?mbox foaf:name ?mboxName} }"
    		...
    		//"PREFIX dc10:  <http://purl.org/dc/elements/1.0/> PREFIX dc11:  <http://purl.org/dc/elements/1.1/> SELECT ?title WHERE  { { ?book1 dc10:title  ?title } UNION { ?book2 dc11:title  ?title } }"
        );
    	try {
